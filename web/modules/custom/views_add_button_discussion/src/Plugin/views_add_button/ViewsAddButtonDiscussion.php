...
class ViewsAddButtonDiscussion extends PluginBase implements ViewsAddButtonInterface {
  ...
  public static function generateUrl($entity_type, $bundle, array $options, $context = '') {
    ...
    $options['query']["edit[field_discussion_source][widget][0][target_id]"] = $context;
    $query = array_map(function ($key, $value) {
      return "{$key}={$value}";
    }, array_keys($options['query']), $options['query']);

    //    $url = Url::fromUri("internal:/node/add/{$bundle}?" . implode('&', $query));
